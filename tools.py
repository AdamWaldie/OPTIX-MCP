...
def _fmt_threat_actor_profile_md(profile: Any) -> str:
    ...
    if entity.description:
        ...
        lines.append(entity.description)
        lines.append(
            "\n_This summary was generated by a previous `entity.research` job. "
            "Call `entity.research` again to refresh it (costs 15 credits)._"
        )
        lines.append("")
    ...
    if profile.linked_report_count > 0:
        ...
        if profile.latest_report_summary and profile.latest_report_id:
            ...
            lines.append(
                f"\n_Retrieve the full document with `report.get` "
                f"using `report_id: {profile.latest_report_id}`. "
                f"See all linked documents with `report.list` and `entity_id: {entity.id}`._"
            )
        else:
            lines.append(
                f"\n_Documents are present but no summary is cached. "
                f"Use `report.list` with `entity_id: {entity.id}` "
                "to retrieve report IDs, then `report.get` with a report_id._"
            )
    else:
        lines.append(
            "No intelligence documents are linked to this actor yet. "
            "Call `entity.research` to trigger a deep AI research job (costs 15 credits) "
            "or `search.documents` to find relevant open-source documents."
        )
    lines.append("")
    ...
    lines.append("---")
    lines.append(
        "_To trigger deep AI research on this actor, call `entity.research` with "
        f"`entity_id: {entity.id}` (costs 15 credits). "
        f"To generate a hunting query, call `entity.ask` with `entity_id: {entity.id}`._"
    )
...
def register_tools(server: Server) -> None:
        ...
        tools = [
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "Use `document.get` to fetch the full detail for any entry returned here. "
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "`ioc.context`, which requires that ID for enrichment lookup. "
                    "This tool is free (no credit cost)."
                ),
                annotations=types.ToolAnnotations(
                    readOnlyHint=True,
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "NOTE: This operation costs 40 OPTIX credits. Call `account.status` first if "
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "TIP: For threat actor queries specifically, prefer `actor.profile` "
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "(incident.report: 40 credits; report.generate: 40 credits; entity.research: "
                    "15 credits; entity.ask / detection.rule / detection.tradecraft: "
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "Get the document ID from `feed.get` (feed entries include IDs), "
                    "`search.documents` results, or the `id` field in any intelligence document reference. "
                    "This tool is free (no credit cost)."
                ),
                annotations=types.ToolAnnotations(
                    readOnlyHint=True,
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "malware family — entity IDs come from `entity.get` or `actor.profile`. "
                    "Use `report.get` with the returned report ID to read the full text. "
                    "This tool is free (no credit cost)."
                ),
                annotations=types.ToolAnnotations(
                    readOnlyHint=True,
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "`report.list`, `report.generate`, or the `latest_report_id` "
                    "field in `actor.profile`. "
                    "This tool is free (no credit cost)."
                ),
                annotations=types.ToolAnnotations(
                    readOnlyHint=True,
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "Pair with `detection.rule` to create rules for uncovered techniques. "
                    "This tool is free (no credit cost)."
                ),
                annotations=types.ToolAnnotations(
                    readOnlyHint=True,
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "Use this before `watchlist.remove` to confirm which entities are watched "
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "(ID is returned by `entity.get`, `actor.profile`, or any search result). "
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "notifications about it. Requires the numeric entity ID — use `watchlist.list` "
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "To get a full profile for a card actor, call `actor.profile`. "
                    "This tool is free (no credit cost)."
                ),
                annotations=types.ToolAnnotations(
                    readOnlyHint=True,
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "Requires a numeric IOC entity ID — use `search.indicator` first to find the ID "
                ...
                inputSchema={
                    ...
                    "properties": {
                        "ioc_id": {
                            "type": "integer",
                            "description": "Numeric OPTIX entity identifier for the IOC. Use search.indicator to find the ID.",
                            "minimum": 1,
                        },
                    },
                    "required": ["ioc_id"],
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "Use after fetching a document (via `document.get` or `search.documents`) to "
                    "understand your detection blind spots for that threat report. "
                    "Pair with `detection.rule` to close gaps for specific technique IDs. "
                    "This tool is free (no credit cost)."
                ),
                annotations=types.ToolAnnotations(
                    readOnlyHint=True,
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "Campaign, Technique. Requires a numeric entity ID — use `entity.get` or "
                    "`actor.profile` first to find the ID. "
                    "WHEN TO ESCALATE: Use this only when `actor.profile` or `entity.get` "
                    ...
                    "NOTE: This operation costs 15 OPTIX credits. Call `account.status` first "
                ...
                inputSchema={
                    ...
                    "properties": {
                        "entity_id": {
                            "type": "integer",
                            "description": "Numeric OPTIX entity identifier. Use entity.get to find it.",
                            "minimum": 1,
                        },
                        "force_refresh": {
                            "type": "boolean",
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "Use `entity.get` or `actor.profile` first to find the numeric entity ID. "
                    "NOTE: This operation costs 4 OPTIX credits. Call `account.status` first "
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "Find technique IDs from `detection.attack_matrix` or `detection.coverage_gaps`. "
                    f"Supported SIEM languages: {_SIEM_LANG_CHOICES}. "
                    "NOTE: This operation costs 4 OPTIX credits. Call `account.status` first "
                    "if unsure about the balance."
                ),
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "Unlike `detection.rule` (which targets a single technique), this tool "
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "ID that can be retrieved with `report.get` once generation completes. "
                    "NOTE: This operation costs 40 OPTIX credits. Call `account.status` first if "
                    "you are unsure whether the account has sufficient credits."
                ),
                annotations=types.ToolAnnotations(
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "actor, call `entity.research` afterwards (15 credits). To generate a SIEM "
                    "hunting query, call `entity.ask` (4 credits)."
                ),
                annotations=types.ToolAnnotations(
                    readOnlyHint=True,
                    destructiveHint=False,
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "To get the full profile for a specific actor, call `actor.profile`. "
                    "This tool is free (no credit cost)."
                ),
                annotations=types.ToolAnnotations(
                    readOnlyHint=True,
            ...
            types.Tool(
                ...
                description=(
                    ...
                    "IOC entity IDs come from `search.indicator` or the numeric id in any IOC context result. "
                    "This tool is free (no credit cost)."
                ),
                annotations=types.ToolAnnotations(
                    readOnlyHint=False,
    ...
    async def call_tool(name: str, arguments: dict[str, Any]) -> list[types.TextContent]:
        ...
        except OptixAuthError:
            return [
                types.TextContent(
                    type="text",
                    text=(
                        "Tool invocation requires an OPTIX API key.\n\n"
                        "Please provide your API key in the X-API-Key request header "
                        "(or as Authorization: Bearer <key>) when connecting to the MCP server.\n\n"
                        "You can generate an API key from your OPTIX account settings at "
                        "https://optixthreatintelligence.co.uk."
                    ),
                )
            ]
        ...
        try:
            ...
            elif name == "detection.attack_matrix":
            ...
            elif name == "detection.coverage_gaps":
            ...
            elif name == "detection.tradecraft":
                _check_credits(4)
